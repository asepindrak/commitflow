...
function safeString(v: any): string {
    ...
    if (typeof v === 'object') {
        ...
        if (typeof v.fullName === 'string' && v.fullName.trim() !== '') return v.fullName;
...
export function normalizeTeamInput(raw: any[] = []): TeamMember[] {
    return (raw || []).map((t: any, idx: number) => {
        if (typeof t === 'string' && t.trim() !== '') {
            return { id: `tm_${idx}_${Math.random().toString(36).slice(2, 6)}`, name: t.trim() };
        }
        ...
        return { id: `tm_${idx}_${Math.random().toString(36).slice(2, 6)}`, name: safeString(t).trim() };
    });
}
