      ...
      <div className="flex items-center gap-3 flex-wrap mb-4">
            ...
            <div className="absolute top-full left-0 mt-1.5 z-30 bg-white dark:bg-gray-900 border border-gray-100 dark:border-gray-700 rounded-2xl shadow-xl shadow-gray-200/60 dark:shadow-black/40 p-3 min-w-[200px]">
              ...
              <div className="flex flex-col gap-1">
                {STATUS_OPTIONS.map((opt) => (
                  ...
                  >
                    <span
                      className={`w-2 h-2 rounded-full shrink-0 ${filterStatuses.includes(opt.key as string) ? "opacity-100" : "opacity-30"} ${opt.color.split(" ")[0].replace("bg-", "bg-")}`}
                    />
                    {opt.label}
                    {filterStatuses.includes(opt.key as string) && (
        ...
        <div className="relative">
          ...
          >
            ...
            <ChevronDown
              size={13}
              className={`transition-transform ${showPriorityFilter ? "rotate-180" : ""}`}
            />
          </button>
          ...
          <div className="flex items-center gap-1.5 flex-wrap">
            {filterStatuses.map((s) => {
              ...
              return (
                <span
                  key={s}
                  className={`inline-flex items-center gap-1 pl-2 pr-1 py-0.5 rounded-full text-xs font-medium ${opt?.color ?? ""}`}
                >
                  {opt?.label}
                  <button
