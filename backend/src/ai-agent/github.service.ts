...
export async function getContributors(repo: string, retries = 3) {
  ...
  try {
    ...
    for (let attempt = 1; attempt <= retries; attempt++) {
      ...
      if (res.status === 202) {
        console.log(
          `Stats for ${owner}/${repo} are not ready yet. Retrying (${attempt}/${retries})...`
        );
      ...
      if (res.status === 404) {
        console.log(
          `Repository ${owner}/${repo} not found or stats not available.`
        );
        return [];
      }
    ...
    console.log(
      `Stats for ${owner}/${repo} still not ready after ${retries} retries.`
    );
    return [];
  } catch (error) {
  ...
  while (true) {
    ...
    for (let attempt = 1; attempt <= retries; attempt++) {
      try {
        ...
        console.log(
          `Error fetching page ${page}, attempt ${attempt}/${retries}:`,
          err.message
        );
        if (attempt < retries) {
    ...
    if (!success) {
      console.log(
        `Failed to fetch page ${page} after ${retries} retries. Stopping.`
      );
      break;
    }
